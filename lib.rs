...
mod clinical_trial_data {
    ...
    impl ClinicalTrialData {
        ...
        #[ink(message)]
        pub fn upload_preprocessed(&mut self, records: Vec<(u64, String, String)>) {
            for record in records {
    ...
    mod tests {
        ...
        #[ink::test]
        fn upload_download_process_works() {
            let sample: Vec<(u64, String, String)> = vec![(1, "Treatment".to_string(), "Yes".to_string()), (2, "Placebo".to_string(), "No".to_string())];
            
            let mut research = ClinicalTrialData::default();

            research.upload_raw(sample.clone());
            ink_env::debug_println!("raw records: {:?}", research.raw_records);
            assert!(research.raw_records == sample);

            let download = research.download_raw();
            ink_env::debug_println!("downloaded records: {:?}", download);
            assert!(download == research.raw_records);

            research.upload_preprocessed(sample.clone());
            ink_env::debug_println!("preprocessed records: {:?}", research.preprocessed_records);
            assert!(research.preprocessed_records == sample);

            ink_env::debug_println!("treatment positive: {:?}", research.data_summary.get(&String::from("Treatment Positive")).unwrap());
            assert!(research.data_summary.get(&String::from("Treatment Positive")).unwrap() == 1);
            assert!(research.data_summary.get(&String::from("Placebo Negative")).unwrap() == 1);
