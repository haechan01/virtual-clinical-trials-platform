...
mod clinical_trial_data {
    ...
    mod tests {
        ...
        macro_rules! vec_of_strings {($(($x:expr), ($y:expr), ($z:expr)),*) => (vec![$(($x), ($y.to_string()), ($z.to_string())),*]);}
        ...
        fn upload_raw_works() {
            let research = ClinicalTrialData::default();
            research.upload_raw(SAMPLE[..100].to_vec());
            assert!(research.raw_records == &SAMPLE[..100])
        }
    }
}
